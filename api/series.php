...
foreach($portals as $portal){
    ...
    $res = trim($res);
    if($res === false || $res == '{"user_info":{"auth":0}}' || $res == header('404 (Not Found)') || stripos($res,"HTTP/1.1 401 Unauthorized") || $res=='{"success": true}') {
        // SO for http://system52.xyz/ it fails when you get {"success": true}
        continue;
    }
    else{
